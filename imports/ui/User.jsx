...
const User = ({ data, deletUser }) => {

    const { name, rut, lasNameFather, lasNameMother, comuna, region, _id } = data;

    const handleClick = (_id) => {
        const option = confirm('Estas seguro de eleminar un paciente registrado')
        if (option === false) {
            return
        } else {
            deletUser(_id)
    ...
    return (
        <div className='grid grid-cols-12 gap-1 my-2  bg-stone-50/50 p-1 rounded-sm text-center text-sm'>


            <p className="mb-1">{rut}</p>
            <p className="mb-1 col-span-2">{name}</p>
            <p className="mb-1 col-span-2">{lasNameFather}</p>
            <p className="mb-1 col-span-2">{lasNameMother}</p>
            <p className="mb-1 col-span-2">{region}</p>
            <p className="mb-1 col-span-2">{comuna}</p>
            <div className="mb-1 flex flex-row  items-center justify-center ">
                <button className='hover:scale-110 ease-in-out duration-300' onClick={() => handleClick(_id)}>
